...
def generate_txt_path(pdf_path, output_directory):
    pdf_basename = os.path.basename(pdf_path)
    pdf_name, _ = os.path.splitext(pdf_basename)
    txt_path = os.path.join(output_directory, f"{pdf_name}_output.txt")
    return txt_path
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor() as executor:
        ...
        executor.map(lambda pdf_batch: convert_batch(pdf_batch, pdf_directory, output_directory), batches)
